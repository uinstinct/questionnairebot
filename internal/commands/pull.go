...
func (p *Pull) Handle(ctx context.Context, sender bot.Sender) error {
	...
	for _, slug := range slugs {
		...
		if _, err := ApplyPastDueSkips(ctx, p.Flow.DataDir, q, now, p.Clock); err != nil {
		...
		last, _ := storage.LastEntry(ctx, p.Flow.DataDir, slug)
...
func (p *Pull) HandleCallback(ctx context.Context, sender bot.Sender, data string) error {
	...
	return p.Flow.StartQuestionnaire(ctx, slug, scheduled)
}
